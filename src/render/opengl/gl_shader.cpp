...
int Shader::get_uniform_location(const std::string& name)
{
    ...
    if (location == -1)
        CPPSIM_TRACE("Uniform {0} does not exist", name);
...
unsigned Shader::compile_shader(unsigned type, const std::string& source)
{
    ...
    if (!success)
    {
        ...
        CPPSIM_TRACE(info_log);
        CPPSIM_ASSERT(false, "Shader compilation failed");
    }

    CPPSIM_TRACE("Shader compilation succes");
...
ShaderProgramSource Shader::parse_shader()
{
    ...
    CPPSIM_ASSERT(!stream.fail(), "File could not be opened");
