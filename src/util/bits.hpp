...
{
    CPPSIM_ASSERT(from > 0 && from <= 16, "Invalid from range");
    CPPSIM_ASSERT(to > 0 && to <= 16, "Invalid to range");
    CPPSIM_ASSERT(from <= to, "From must be smaller than or equal to to");
